...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    public boolean isBalanced(Node<E> node) {
        return (node == null) || isBalanced(node.getLeftChild()) &&
                ...
                Math.abs(getHeight(node.getLeftChild()) - getHeight(node.getRightChild())) <= 1;
    }

    @Override
    public int getHeight(Node<E> node) {
        return node == null ? 0 : 1 + Math.max(getHeight(node.getLeftChild()), getHeight(node.getRightChild()));
    }

    @Override
    public int getDeep(E value) {
        Node<E> current = root;
        int currentDeep = 1;
        while (current != null) {
    ...
    public boolean add(E value) {
        ...
        if (previous != null && getDeep(previous.getValue()) == maxHeight) {
            return false;
        } else {
            if (previous == null) {
                root = node;
